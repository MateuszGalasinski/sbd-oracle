...
docker login container-registry.oracle.com --username $USER -p $PASS
docker rm -f $containerName 2>&1>/dev/null # delete old one if exists
...
echo "---> Container's ready to connect on localhost:1521 !"
